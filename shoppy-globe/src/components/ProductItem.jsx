...
function ProductItem({ product }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden w-64 h-[420px] flex flex-col hover:shadow-lg transition-shadow duration-300 ">
      {/* Image */}
      <Link to={`/ProductDetails/${product.id}`}><div className="h-40 w-full bg-gray-100">
        <img
      ...
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div className="flex flex-col gap-2">
          <Link to={`/ProductDetails/${product.id}`}> <h2 className="text-lg font-semibold text-gray-800 truncate">
            {product.title}
          </h2></Link>
        ...
        <div className="mt-auto pt-3">
          ...
          {/* Add to Cart button */}
         {/* <Link  to="/Cart"> */}
          <button onClick={()=>addToCart(product)} className="w-full bg-[#FFD814] text-gray-900 py-2 rounded-full font-semibold hover:bg-[#F7CA00] transition-colors duration-200 cursor-pointer">
